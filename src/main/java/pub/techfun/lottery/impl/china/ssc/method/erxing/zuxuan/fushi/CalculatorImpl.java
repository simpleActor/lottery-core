package pub.techfun.lottery.impl.china.ssc.method.erxing.zuxuan.fushi;
...
public class CalculatorImpl extends SSCAbstractCalculator<BetImpl> {
    ...
    private static final double odds = SSCConfig.ALL_FACTOR / 1000;
    ...
    public BetRegion cover(String bet) {
        ...
        List<List<Integer>> lists = ACUtil.A(betWrapper.getBet().get(0).getNumList(), 2);
    ...
    public double coverRate(String bet) {
        return newBetWrapper(bet).getCount() * 2 * 1000 / SSCConfig.ALL_FACTOR;
    }
    ...
    protected void processWinJudge(WinNumberGroup winNumber, BetWrapper betWrapper, WinResultGroup result) {
        if (winNumber.first().get(0).equals(winNumber.first().get(1))) {
            //开奖号码两位相同，直接返回
            return;
        }
        if (betWrapper.getBet().get(0).contains(winNumber.first().getNumList())) {
