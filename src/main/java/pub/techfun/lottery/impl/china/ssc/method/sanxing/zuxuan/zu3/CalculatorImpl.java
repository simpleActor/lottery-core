package pub.techfun.lottery.impl.china.ssc.method.sanxing.zuxuan.zu3;
...
public class CalculatorImpl extends SSCAbstractCalculator<BetImpl> {
    ...
    private static final double odds = SSCConfig.ALL_FACTOR / 300;
    ...
    public BetRegion cover(String bet) {
        ...
        List<List<Integer>> lists = ACUtil.C(betWrapper.getBet().get(0).getNumList(), 2);
        List<List<Integer>> numLists = new ArrayList<>();
        lists.forEach(list -> {
            List<Integer> nums1 = new ArrayList<>(list);
            nums1.add(list.getFirst());
            List<Integer> nums2 = new ArrayList<>(list);
            nums2.add(list.get(1));
            numLists.addAll(ACUtil.A(nums1, 3));
            numLists.addAll(ACUtil.A(nums2, 3));
        });
        List<BetSection> fullSectionList = CoverUtil.getFullCoverByNumLists(this.indexes, numLists.stream().distinct().toList());
        return new BetNumRegion().addAll(fullSectionList);
    }

    @Override
    public double coverRate(String bet) {
        return newBetWrapper(bet).getCount() * 300 / SSCConfig.ALL_FACTOR;
    }
    ...
    protected void processWinJudge(WinNumberGroup winNumber, BetWrapper betWrapper, WinResultGroup result) {
        ...
        if (betWrapper.getBet().get(0).contains(repeatNum.getFirst())
                && betWrapper.getBet().get(0).contains(singleNum.getFirst())) {
