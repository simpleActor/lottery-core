package pub.techfun.lottery.impl.china.ssc.method.wuxing.zuxuan.zuxuan5;
...
public class CalculatorImpl extends SSCAbstractCalculator<BetImpl> {
    ...
    public double coverRate(String bet) {
        return newBetWrapper(bet).getCount() * 5 / SSCConfig.ALL_FACTOR;
    }
    ...
    protected void processWinJudge(WinNumberGroup winNumber, BetWrapper betWrapper, WinResultGroup result) {
        ...
        NumSet repeat4Num = countMap.getByCount(4);
        NumSet singleNum = countMap.getByCount(1);
        if(repeat4Num == null || singleNum == null || repeat4Num.size() != 1 || singleNum.size() != 1) return;
        if(betWrapper.getBet().get(0).contains(repeat4Num.getFirst())
                && betWrapper.getBet().get(1).contains(singleNum.getFirst())) {
