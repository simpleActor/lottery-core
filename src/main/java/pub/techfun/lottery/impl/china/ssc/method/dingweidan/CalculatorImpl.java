package pub.techfun.lottery.impl.china.ssc.method.dingweidan;
...
public class CalculatorImpl extends SSCAbstractCalculator<BetImpl> {
    ...
    private static final double odds = SSCConfig.ALL_FACTOR / 10000;
    ...
    public double coverRate(String bet) {
        //乘以1000转换为5星注数
        return newBetWrapper(bet).getCount() * 10000 / SSCConfig.ALL_FACTOR;
    }
    ...
    protected void processWinJudge(WinNumberGroup winNumber, BetWrapper betWrapper, WinResultGroup result) {
        if (betWrapper.getBet().get(0).contains(winNumber.first().get(0))) {
