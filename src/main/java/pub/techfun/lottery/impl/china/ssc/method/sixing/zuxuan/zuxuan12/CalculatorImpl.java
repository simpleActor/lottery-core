package pub.techfun.lottery.impl.china.ssc.method.sixing.zuxuan.zuxuan12;
...
public class CalculatorImpl extends SSCAbstractCalculator<BetImpl> {
    ...
    private static final double odds = SSCConfig.ALL_FACTOR / 10 / 12;
    ...
    public BetRegion cover(String bet) {
        ...
        betWrapper.getBet().get(0).getNumList().forEach(repeatNum -> {
            ...
            cList.forEach(singleNums -> {
                if (singleNums.contains(repeatNum)) return;
                List<Integer> numList = new ArrayList<>();
                //重号
                numList.add(repeatNum);
                numList.add(repeatNum);
                //单号
                numList.addAll(singleNums);
                //求排列并去重
    ...
    public double coverRate(String bet) {
        //4星组选12，先乘以12得到4星复式注数，再乘以10转换为5星注数
        return newBetWrapper(bet).getCount() * 12 * 10 / SSCConfig.ALL_FACTOR;
    }
