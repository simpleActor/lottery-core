...
class BetImpl extends SSCAbstractBetWrapper {
     ...
     * section 约束
     */
    public static final SectionParam SECTION_CONSTRAINT = new SectionParam(0, 18, 1, 19, false, false);
    ...
    protected void calculateCount() {
        this.count = getBet().get(0).getNumList().stream().map(val -> SumACUtil.getSumNCountA(2, val))
                .reduce(0L, Long::sum);
    }
