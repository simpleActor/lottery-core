package pub.techfun.lottery.impl.china.ssc.method.sixing.zhixuan.zuhe;
...
public class CalculatorImpl extends SSCAbstractCalculator<BetImpl> {
    ...
    private static final double odds1 = SSCConfig.ALL_FACTOR / 10;
    private static final double odds2 = SSCConfig.ALL_FACTOR / 100;
    private static final double odds3 = SSCConfig.ALL_FACTOR / 1000;
    private static final double odds4 = SSCConfig.ALL_FACTOR / 10000;
    ...
    public double coverRate(String bet) {
        //4星组合计算注数时需要乘以4，这里先除以4得到4星复式注数，再乘以10转换为5星注数
        return newBetWrapper(bet).getCount() / 4d * 10 / SSCConfig.ALL_FACTOR;
    }
    ...
    protected List<WinResult> newWinResultList() {
        return List.of(new SSCWinResult(odds1), new SSCWinResult(odds2), new SSCWinResult(odds3), new SSCWinResult(odds4));
    }
