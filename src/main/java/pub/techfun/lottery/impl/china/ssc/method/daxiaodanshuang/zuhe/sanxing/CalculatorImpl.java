package pub.techfun.lottery.impl.china.ssc.method.daxiaodanshuang.zuhe.sanxing;
...
public class CalculatorImpl extends SSCAbstractCalculator<BetImpl> {
    ...
    private static final double odds = 8;
    ...
    public BetRegion cover(String bet) {
        ...
        List<List<Integer>> result = lists.stream().filter(list -> {
            ...
            //第三位
            if (betWrapper.getBet().get(2).get(0) == 1) {
                //大
                return list.get(2) > 4;
            }
            if (betWrapper.getBet().get(2).get(1) == 1) {
                //小
                return list.get(2) < 5;
            }
            if (betWrapper.getBet().get(2).get(2) == 1) {
                //单
                return list.get(2) % 2 != 0;
            }
            if (betWrapper.getBet().get(2).get(3) == 1) {
                //双
                return list.get(2) % 2 == 0;
            }
            return false;
    ...
    protected void processWinJudge(WinNumberGroup winNumber, BetWrapper betWrapper, WinResultGroup result) {
        ...
        //第三位
        if (betWrapper.getBet().get(2).get(0) == 1 && winNumber.first().get(2) > 4) {
            //大
            match3++;
        }
        if (betWrapper.getBet().get(2).get(1) == 1 && winNumber.first().get(2) < 5) {
            //小
            match3++;
        }
        if (betWrapper.getBet().get(2).get(2) == 1 && winNumber.first().get(2) % 2 != 0) {
            //单
            match3++;
        }
        if (betWrapper.getBet().get(2).get(3) == 1 && winNumber.first().get(2) % 2 == 0) {
            //双
            match3++;
        }
        result.first().setWinCount(match1 * match2 * match3);
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < betCount; i++) {
            ...
            //排除错误注单
            if(!bet.get(0).contains(1) || !bet.get(1).contains(1) || !bet.get(2).contains(1)) continue;
            betList.add(bet);
        }
