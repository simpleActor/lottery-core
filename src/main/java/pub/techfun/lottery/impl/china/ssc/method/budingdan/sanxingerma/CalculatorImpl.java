package pub.techfun.lottery.impl.china.ssc.method.budingdan.sanxingerma;
...
public class CalculatorImpl extends SSCAbstractCalculator<BetImpl> {
    ...
    private static final double odds = SSCConfig.ALL_FACTOR / 100 / 54;
    ...
    public BetRegion cover(String bet) {
        ...
        List<List<Integer>> bets = ACUtil.C(betWrapper.getBet().get(0).getNumList(), 2);
        lists.forEach(list -> {
    ...
    protected void processWinJudge(WinNumberGroup winNumber, BetWrapper betWrapper, WinResultGroup result) {
        int winCount = 0;
        List<List<Integer>> winNumberLists = ACUtil.C(winNumber.first().getNumList(), 2);
