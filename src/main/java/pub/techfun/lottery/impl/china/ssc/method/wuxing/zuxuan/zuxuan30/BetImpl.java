...
class BetImpl extends SSCAbstractBetWrapper {
    ...
    protected void calculateCount() {
        for (Integer singleNum : getBet().get(1).getNumList()) {
            if(getBet().get(0).contains(singleNum)) {
                this.count += ACUtil.countC(getBet().get(0).size() - 1, 2);
            } else {
                this.count += ACUtil.countC(getBet().get(0).size(), 2);
            }
        }
    }
