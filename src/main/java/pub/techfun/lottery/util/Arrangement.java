...
public class Arrangement implements Serializable {
     ...
     * @param datas 用于排列的数据
     */
    public Arrangement(Integer[] datas) {
        this.datas = datas;
    }
     ...
     * @return 所有排列列表
     */
    public List<Integer[]> select(int m) {
        final List<Integer[]> result = new ArrayList<>((int) count(this.datas.length, m));
        select(this.datas, new Integer[m], 0, result);
        return result;
    }
    ...
    public List<Integer[]> selectAll() {
        final List<Integer[]> result = new ArrayList<>((int) countAll(this.datas.length));
     ...
     * @param result      最终结果
     */
    private void select(Integer[] datas, Integer[] resultList, int resultIndex, List<Integer[]> result) {
